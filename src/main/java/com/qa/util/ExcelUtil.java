...
public class ExcelUtil {
  ...
  public ExcelUtil(String path) {
    this.path = path;
    try {
      loggerUtil = new LoggerUtil(ExcelUtil.class);
  ...
  public String getCellData(String sheetName, String colName, int rowNum) {
    ...
    try {
      ...
      if (rowNum <= 0 || sheet.getRow(rowNum - 1) == null) return "";
      ...
      if (cell.getCellType().name().equals("STRING")) return cell.getStringCellValue();
        ...
        return cellText;
      } else if (cell.getCellType().name().equals("BLANK")) return "";
  ...
  /**
   * {@summary returns true if sheet is removed successfully else false if sheet does not exist}
   *
   * @param
   * @return
   * @author deepak rai
